...
plotData <- aggregate(DataOnReleaseYear$count,
                      by=list(DataOnReleaseYear$RELEASE_YEAR,
                              DataOnReleaseYear$IS_CONGRUENT),
                      FUN=sum)
colnames(plotData) <- c("RELEASE_YEAR", "IS_CONGRUENT", "TOTAL")
...
plotData$PERCENTAGE <- as.numeric(sqldf("SELECT (plotData.TOTAL/acc_per_year.TOTAL) FROM plotData,
                                        acc_per_year WHERE plotData.RELEASE_YEAR = acc_per_year.RELEASE_YEAR")[,1])
...
## PLOTTING OPERATIONS ##
#ggplot(data=plotData, mapping = aes(x=RELEASE_YEAR, y=PERCENTAGE)) +
#        geom_col(aes(fill=IS_CONGRUENT)) +
#        coord_flip() # coord_flip because otherwise the year labels overlap (can be fixed with wider bars?)
