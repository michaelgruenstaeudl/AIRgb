...
year_plot = ggplot(data=plotData_year, aes(x=RELEASE_YEAR, y=PERCENTAGE), width=1) +
  ...
  scale_fill_manual(values=c("grey0", "grey50"),
                    name="Presence of annotations and length equality",
                    labels=c("No", "Yes")) +
  theme_minimal() + 
...
comb_plot <- ggarrange(year_plot, publication_plot, seqn_plot, nrow=1, ncol=3, common.legend=TRUE, legend="bottom", widths = c(8,2,3))
ggsave(file = "05_FIGURE_CompositeIRequalityComparison.pdf", plot=comb_plot, width=11)
#gt = gtable(widths = unit(c(2,0.5,0.75, .3), "null"), height = unit(1, "null"))
#gt <- gtable_add_grob(gt, ggplotGrob(year_plot), 1, 1)
#gt <- gtable_add_grob(gt, ggplotGrob(publication_plot), 1, 2)
#gt <- gtable_add_grob(gt, ggplotGrob(seqn_plot), 1, 3)
