...
ggplot(data=plotData, mapping = aes(x=YEAR, y=TOTAL)) + geom_col(aes(fill=IS_CG)) + coord_flip() # coord_flip because otherwise the year labels overlap (can be fixed with wider bars?)