...
def confirmDifferentStartEnd(feature, log):
    ...
    if feature:
        if feature.location.start == feature.location.end:
            ...
            log.debug("Adjusted FeatureLocation to %s" % str(feature.location))
    return feature
...
def constructIRsFromJunctions(rec_len, jlb_feat, jsb_feat, jsa_feat, jla_feat, log):
    ...
    for junction in [jlb_feat, jsb_feat, jsa_feat, jla_feat]:
        junction = confirmDifferentStartEnd(junction, log)
        ...
        if IRa is None:
            if any(identifier in misc_feature.qualifiers["note"][0].lower() for identifier in ira_identifiers) and \
              not any(blocked in misc_feature.qualifiers["note"][0].lower() for blocked in blacklist):
                log.debug("Found identifier for IRa: " % str(misc_feature.qualifiers["note"][0]))
                if len(misc_feature) > 100:
                    IRa = misc_feature
                else:
                    log.debug("Feature is too short (%s bp) to be an IR." % str(len(misc_feature)))
        if IRb is None:
            if any(identifier in misc_feature.qualifiers["note"][0].lower() for identifier in irb_identifiers) and \
              not any(blocked in misc_feature.qualifiers["note"][0].lower() for blocked in blacklist):
                log.debug("Found identifier for IRb: %s" % str(misc_feature.qualifiers["note"][0]))
                if len(misc_feature) > 100:
                    IRb = misc_feature
                else:
                    log.debug("Feature is too short (%s bp) to be an IR." % str(len(misc_feature)))
    ...
    if IRa is None or IRb is None:
        for misc_feature in [mf for mf in all_mf_no_pseudo if "note" in mf.qualifiers]:
            if (("inverted" in misc_feature.qualifiers["note"][0].lower() and \
              ...
              not any(blocked in misc_feature.qualifiers["note"][0].lower() for blocked in blacklist):
                log.debug("Found general identifier for IRs: %s" % str(misc_feature.qualifiers["note"][0]))
                if len(misc_feature) > 100:
                    if IRb is None:
...
def getInvertedRepeats(rec, log, min_IR_len=1000):
    '''
    ...
    for repeat_feature in [rf for rf in all_repeat_features if "rpt_type" in rf.qualifiers]:
        i += 1
        log.debug("Checking repeat_feature %s out of %s (position %s - %s)..." % \
        ...
        if repeat_feature.qualifiers["rpt_type"][0].lower() == "inverted":
            ...
            if len(repeat_feature) > min_IR_len:
                ...
                log.info("Inverted repeat feature detected at position %s - %s. Region is too small (smaller than %s bp) to be IRa or IRb." % \
                  (str(repeat_feature.location.start), str(repeat_feature.location.end), str(min_IR_len)))
    if IRa is None or IRb is None:
        log.debug("%s out of 2 IR positions found so far. Checking repeat_features without 'rpt_type' qualifier." % \
        ...
        for repeat_feature in [feature for feature in all_repeat_features if not "rpt_type" in feature.qualifiers]:
            ...
            if "note" in repeat_feature.qualifiers:
                ...
                if any(identifier in repeat_feature.qualifiers["note"][0].lower() for identifier in ira_identifiers):
                    ...
                    log.info("Found a repeat region (%s - %s) without further identifying information. Ignoring this feature." % \
                      (str(repeat_feature.location.start), str(repeat_feature.location.end)))
    # Sanity check for IRs selected by the script so far
    if (IRa is not None and len(IRa.extract(rec).seq) < min_IR_len) or \
       (IRb is not None and len(IRb.extract(rec).seq) < min_IR_len):
        log.warning("Selected IRs are too short to be genuine IRs and have been discarded.")
        IRa = None
        IRb = None
    ...
    if IRa is None or IRb is None:
        ...
        if IRa is None or IRb is None:
            log.debug("%s out of 2 IR positions found so far. Checking all misc_features for JUNCTION INFORMATION..." % \
              (str([IRa is None, IRb is None].count(False))))
            ...
            for misc_feature in [mf for mf in all_misc_features if "note" in mf.qualifiers]:
                i += 1
                log.debug("Checking misc_feature %s out of %s (position %s - %s)..." % \
    ...
    if os.path.isfile(args.outfn):
        with open(args.outfn) as outfile:
            ...
            if not (header == "\t".join(columns) + "\n" or \
                    header == "\t".join(columns + additional_columns)):
        ...
        IRinfo_table = pd.DataFrame(columns=columns[1::])
        # Note: "ACCESSION" is omitted as column name, because it will be set as name of the index column
            ...
            try:
                ...
                if IRa_feature and IRbRC_feature:
                    score_noRC = fuzz.ratio(IRa_feature.extract(rec).seq,
                                            IRbRC_feature.extract(rec).seq)
                    score_RC = fuzz.ratio(IRa_feature.extract(rec).seq,
                                          IRbRC_feature.extract(rec).seq.reverse_complement())
